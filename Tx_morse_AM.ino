...
const long periodo_transmitir=30; 
...
const int duracion_punto=DURACION_PUNTO;//número de duración para punto
const int pausa_punto=DURACION_PUNTO;//pausa y punto
const int duracion_raya=3*DURACION_PUNTO;//número de duración de raya
const int pausa_raya=DURACION_PUNTO;//pausa y raya 
...
int delay_punto = 500; //0.5 segundos
...
// registro incrementador (reg = reg + 1)
...
void setup()
{
  ...
  lcd.clear();
  lcd.setCursor(0,0);
  lcd.print("Proyec. Telecom.");
  lcd.setCursor(0,1);
  lcd.print("Tx de Codg Morse");
  delay(5000);
  lcd.clear();
  lcd.setCursor(0,0);
  lcd.print("Josue Alexis M G");
  lcd.setCursor(0,1);
  lcd.print("Pakal Moyron Cas");
  delay(3000);
